...
class TemplateScene(ThreeDScene):
    def _initialize_template(self):
        ...
        self.add_fixed_in_frame_mobjects(finki_logo)
...
class ExampleScene(TemplateScene):
    def construct(self):
        ...
        self.play(Write(sphere := Sphere()))
        self.wait(.2)
        self.play(Unwrite(sphere))
        self.wait(1)

        self.stop_ambient_camera_rotation()

        # Render the animation by writing the following in the terminal:
        #   manim starter_code_3d.py ExampleScene -pqm


# Additionally, for lengthier videos, you may define multiple scenes to ease with the rendering process
class ExampleScene2(TemplateScene):
    def construct(self):
        self.initialize_scene(
            title=['This is an example title', 'displayed on two rows'],
            author='Name Surname',
            skip_intro=True  # Skip the introductory slide as this scene should succeed the ExampleScene
        )

        # Code for this scene goes here
        ...
